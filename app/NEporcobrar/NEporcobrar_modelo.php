...
class NEporcobrar extends Conectar
{
    public function getNEporcobrar($fechai, $fechaf)
    {
        ...
        $sql = "SELECT 
(select Descrip from bconfisur_d.dbo.SACONF) as Empresa,
...
SUM(SAACXC.Saldo) as SubTotal
 from bconfisur_d.dbo.saacxc inner join bconfisur_d.dbo.saclie on saacxc.codclie = saclie.codclie 
...
DATEDIFF(DD, saacxc.fechae, CONVERT( date ,GETDATE()))as DiasTransHoy,
 ...
 (select Coordinador from bconfisur_d.dbo.SAVEND_01 where SAVEND_01.CodVend = saacxc.CodVend) as Supervisor
 from bconfisur_d.dbo.saacxc inner join saclie on saacxc.codclie = saclie.codclie 
